...
import { ArrowRight, CheckCircle2, Cpu, Truck, Leaf, Headphones, Sparkles } from "lucide-react";
...
export default function HomePage() {
    ...
    const whyIcons = [Sparkles, Truck, Leaf, Headphones];
        ...
        <main data-testid="home-page">
                ...
                <div className="container-page pt-12 lg:pt-20 pb-section">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
                        <div className="lg:col-span-7 animate-fade-up">
                            ...
                            >
                                <span className="inline-block w-1.5 h-1.5 rounded-full bg-cobalt" /> {h.badge}
                            </span>
                            ...
                            <dl className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-6 max-w-2xl" data-testid="home-stats">
                                ...
                                <StatBox value="3" label={h.statMachines} />
            ...
            <section className="bg-surface-soft" data-testid="home-services">
                <div className="container-page py-section-lg">
                    <SectionHeader eyebrow={h.sectionServicesEyebrow} title={h.sectionServicesTitle} sub={h.sectionServicesSub} />
                    <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {t.services.list.slice(0, 6).map((s, i) => (
                            <div key={i} className="card-feature group hover:border-ink-deep transition-colors" data-testid={`home-service-card-${i}`}>
                                <div className="flex items-center justify-center w-10 h-10 rounded-pill bg-ink-deep text-canvas mb-5">
                                    <CheckCircle2 className="w-5 h-5" />
                                </div>
                                <h3 className="text-lg font-bold text-ink-deep">{s.t}</h3>
                                <p className="mt-2 text-sm text-ink-charcoal leading-relaxed">{s.d}</p>
                            </div>
                        ))}
                    </div>
            ...
            <section className="bg-canvas" data-testid="home-machines">
                <div className="container-page py-section-lg">
                    <SectionHeader eyebrow={h.sectionMachinesEyebrow} title={h.sectionMachinesTitle} sub={h.sectionMachinesSub} />
                    <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <PhotoCard
                            img={MACHINE_IMG_1}
                            badge="CTP"
                            title="CRON HDI-920 S+"
                            body="Laser imaging direct-to-plate untuk resolusi tinggi dan registrasi yang akurat."
                        />
                        <PhotoCard
                            img={MACHINE_IMG_2}
                            badge="Waterwash"
                            title="Pencucian berbasis air"
                            body="Permukaan plat bersih, konsisten, dengan proses ramah lingkungan."
                        />
                    </div>
                    <div className="mt-10 flex justify-center">
            ...
            <section className="bg-canvas" data-testid="home-cta-banner">
                <div className="container-page py-section-lg">
                    <div className="rounded-4xl bg-ink-deep text-canvas p-10 sm:p-section relative overflow-hidden">
                        <div
                            aria-hidden
                            className="absolute -right-24 -top-24 w-72 h-72 rounded-full bg-cobalt/30 blur-3xl"
                        />
                        <div className="relative grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                            <div>
                                <h2 className="text-display">{h.ctaBannerTitle}</h2>
                                <p className="mt-4 max-w-md text-canvas/80 text-base leading-relaxed">{h.ctaBannerBody}</p>
                            </div>
                            <div className="md:justify-self-end">
                                <Link to="/contact" className="btn-pill-cta" data-testid="home-cta-banner-btn">
                                    {h.ctaBannerBtn} <ArrowRight className="ml-2 w-4 h-4" />
                                </Link>
                            </div>
                        </div>
                    </div>
